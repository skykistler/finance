...
getSymbols('CBOE', src='yahoo', from="2014-05-27", to=pred.date)
...
CBOE.prophet.df <- CBOE.daily[,characteristics]
CBOE.prophet.df %<>%
  mutate(
    y = CBOE.daily$price,
    ds = CBOE.daily$date
...
CBOE.prophet <- prophet(CBOE.prophet.df, daily.seasonality=TRUE, yearly.seasonality=TRUE)

future <- make_future_dataframe(CBOE.prophet, periods = 365)

forecast <- predict(CBOE.prophet, future)
plot(CBOE.prophet, forecast) %>% print
